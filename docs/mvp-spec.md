...
В игре реализована механика супер-сил, которая позволяет пользователям использовать различные упрощения.

Пользователю доступны две супер-силы:
...
Каждая запись в лидерборде содержит имя пользователя, затраченное время и достижения пользователя (прохождение игры без супер-сил). Если пользователь прошёл игру без использования супер-сил, к его позиции в лидерборде добавляется соответствующая ачивка.
